...
This repository is the active cockpit for the Winning Sales Squad Playbook de Vendas.

- The canonical Playbook runtime lives in `07-Runtime-Squad/`.
- Before changing Playbook agents, tasks, workflows, skills, checklists, templates, or runtime rules, read `07-Runtime-Squad/AGENTS.md`.
- The root AIOX install (`.aiox-core/`, `.codex/`, `.claude/`) is an orchestration and IDE layer. It does not replace the markdown-first Playbook runtime.
- Do not add real credentials to `.env`; keep secrets local and private.
