...
This is Thiago Cardoso's personal Second Brain, not a generic software repository.

- Treat AIOX as an orchestration and IDE layer for Claude Code.
- Follow the imported root `AGENTS.md` first for vault behavior, provenance, voice preservation, domain routing, and scoped instructions.
- Use AIOX agents and commands when they improve the actual vault task; do not force story-driven software defaults onto ordinary Markdown knowledge work.
- If asked how AIOX works here, explain the local paths: `.aiox-core/`, `.claude/`, `.codex/`, and the root `AGENTS.md` boundary.
